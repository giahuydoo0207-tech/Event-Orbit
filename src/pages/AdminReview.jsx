...
export default function AdminReview() {
  ...
  const transition = async (eventId, action, reason) => { try { await transitionEventApi(eventId, action, reason); setRejection({ eventId: '', reason: '' }); await load(); setSelectedEvent(null); showToast('Event status updated.', 'success'); } catch (e) { showToast(e.message, 'error'); } };
  ...
  const closeReview = () => { setSelectedEvent(null); setRejection({ eventId: '', reason: '' }); };
  ...
  return <div className="space-y-16 pb-16">
    ...
    <section aria-labelledby="event-review" className="space-y-6">
      ...
      <div className="max-h-[60vh] min-w-0 overflow-x-hidden overflow-y-auto overscroll-contain border-y border-oc-periwinkle/70 md:max-h-[520px]">
        ...
        <div className="divide-y divide-oc-periwinkle/70">
          {data.events.map(event=><EventReviewRow key={event.id} event={event} onReview={setSelectedEvent}/>) }
        </div>
    ...
    {selectedEvent&&<EventReviewModal event={selectedEvent} rejection={rejection} setRejection={setRejection} transition={transition} onClose={closeReview}/>}
  </div>;
}
...
function EventReviewRow({ event, onReview }) {
  ...
  return <article className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_minmax(8rem,0.45fr)_5rem_7rem_5rem] sm:gap-4">
    <div className="min-w-0"><h3 className="truncate text-sm font-black text-oc-ink">{event.name}</h3><p className="mt-1 truncate text-[11px] text-slate-500 sm:hidden">{event.chapter?.name || 'Unknown'} · {event.points ?? 'Not set'} points</p></div>
    <p className="hidden min-w-0 truncate text-xs font-semibold text-slate-600 sm:block">{event.chapter?.name || 'Unknown'}</p>
    ...
    <button className={`${buttonClass} border border-oc-navy text-oc-navy`} onClick={()=>onReview(event)}>Review</button>
...
function EventReviewModal({ event, rejection, setRejection, transition, onClose }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-oc-navy/70 p-0 sm:items-center sm:p-6" onMouseDown={onClose}>
    <section role="dialog" aria-modal="true" aria-labelledby="event-review-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-md bg-white p-5 sm:rounded-md sm:p-7" onMouseDown={event=>event.stopPropagation()}>
      <div className="flex items-start justify-between gap-5 border-b border-oc-periwinkle/70 pb-5">
        <div className="min-w-0"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-oc-blue">Event Review</p><h2 id="event-review-title" className="mt-2 break-words text-2xl font-black text-oc-ink">{event.name}</h2></div>
        <button autoFocus className={`${buttonClass} border border-slate-400 text-slate-700`} onClick={onClose}>Close</button>
      </div>
