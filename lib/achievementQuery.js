...
export async function queryAchievements(supabase, identityColumn, identity) {
  ...
  const { data, error } = await supabase
    .from('achievements')
    .select('id, event_id, user_id, ocid, credential_id, points, tx_hash, token_id, mint_status, checked_in_at, event:events!achievements_event_id_fkey(name, cover_image)')
